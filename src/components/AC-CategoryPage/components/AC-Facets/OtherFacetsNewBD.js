...
const OtherFacets = ({
  ...
  if (numberOfFacetsValuesWithCountMoreThanZero > 0) {
    return (
      <nav key={index} className="facetTitleWrapper ccc dropdownn" id={facetTitle}> 
        <ul>
        {Object.keys(buttonsState[name]).length > 0 ? (
          <li className={facetTitle}>

            {facetTitle.replace("ic ", "").replace("Collection", "Material").replace("Category", "Collection").replace("Tag", "Styles").replace("Stores", "Manufacturers")}
