...
/* const run = async (res) => { */
fs.readdirSync(path.resolve(__dirname, "../../preBuildData/previewProducts"))
  ...
  .map((r) => {
    ...
    try {
      fs.unlinkSync(
        path.resolve(__dirname, `../../preBuildData/previewProducts/${r}`)
      );
      console.error("PRODUCT ", r, " DELETED");
    } catch (err) {
      console.error("Product couldnt deleted", err);
    }
  });
...
fs.writeFile(
  ...
  (err) => {
    ...
    else console.error("PRODUCTS REFRESHED");
  }
);
